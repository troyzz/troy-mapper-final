...
if 'all_photos' not in st.session_state:import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium
import os
...
if 'df' not in st.session_state:
    if os.path.exists(SAVED_DATA):
        ...
        upl = st.file_uploader("Upload CSV (Ticket, Lat, Lon, Notes)", type="csv")
        if upl:
            df = pd.read_csv(upl)
...
# Key="unique_selector" prevents that Duplicate ID error
choice = st.sidebar.selectbox("Jump to Ticket", options=ticket_options, index=current_idx, key="unique_selector")
